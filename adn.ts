					...
					if (options.dlsubs.includes('all') || options.dlsubs.includes(subLang.locale)) {
						...
						if (options.customStyles) {
							...
							subBody =
								...
								'\nYCbCr Matrix: TV.709' +
								'\nCollisions: Normal' +
								`\nScaledBorderAndShadow: ${assParams.ScaledBorderAndShadow || 'yes'}` +
								'\n' +
								'\n[V4+ Styles]' +
							...
							if (assParams.useStylesForPositionAligns) {
								subBody +=
									...
									`\nStyle: ST_UP_ALT,${assParams.fontName},${assParams.textSize},${assParams.textColor},&H000000FF,${
										assParams.borderColor
									},&H00000000,-1,0,0,0,100,100,0,0,1,${assParams.borderSize},${assParams.shadowSize},2,200,200,${assParams.verticalMargin * 2},1` +
						...
						for (const sub of subtitles[subName]) {
							...
							if (options.customStyles?.useStylesForPositionAligns) {
								...
								subBody += `\nDialogue: 0,${this.convertToSSATimestamp(start)},${this.convertToSSATimestamp(end)},${styleNames[alignment]},,0,0,0,,${xtext}`;
							} else {
